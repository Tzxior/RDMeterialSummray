        ...
        try:
            if hasattr(self, 'filePath'):
                ...
                for idx, row in enumerate(self.df.itertuples(index=False), 2):
                    ...
                    self.material_array.append([formatted_row, 1, "", "其他"])
    ...
    def importData(self):
        try:
            ...
            if sheetname_2 in wb.sheetnames:
                response = QMessageBox.question(self, "确认", f"{sheetname_2} 已存在，是否覆盖？", QMessageBox.Yes | QMessageBox.No)
                if response == QMessageBox.No:
                    return
